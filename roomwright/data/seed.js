...
export const SEED_CONSTRAINTS = [
  ...
  },
  {
    ...
    interpretation: 'A midline power bus runs the ship’s length along the overheads; its settling note is part of the ship’s audible anatomy.',
    evidence: 'explicit',
    claims: [{ kind: 'presence', target: 'midlineBus' }],
  },
  {
    key: 'sys-port-duct', source: 'Next-05',
    quote: 'Port duct seam whispered its lie.',
    category: 'sound', subject: 'port air trunk',
    interpretation: 'A port-side air duct runs fore-aft with a seam that hisses faintly — a known, named voice of the ship.',
    evidence: 'explicit',
    claims: [{ kind: 'presence', target: 'portDuct' }],
  },
  {
    key: 'sys-aft-pump', source: 'Next-05',
    quote: 'Aft pump ran two beats late.',
    category: 'sound', subject: 'aft transfer pump',
    interpretation: 'An aft coolant/water pump with a characteristic lag — audible and felt through structure in drift.',
    evidence: 'explicit',
    claims: [{ kind: 'presence', target: 'aftPump' }],
  },
  {
    key: 'nav-locks-helm', source: 'Presence-02',
    quote: 'The nav locks clicked. The forward vector tightened like a belt.',
    category: 'console', subject: 'helm nav locks',
    interpretation: 'The nav locks are physical course-hold hardware at the helm — the click happens under the pilot’s hands in flight (author ruling). The nav compartment on the lower deck is the long-range plotting room; these levers are not it.',
    evidence: 'explicit',
    claims: [{ kind: 'presence', target: 'navLockBank' }],
  },
  {
  ...
  },
  {
    ...
    interpretation: 'The "central prep counter" is the working heart of the galley’s one long integrated counter run — not a free-standing island (author ruling; the phrasing is queued for a light manuscript correction).',
    evidence: 'explicit',
  ...
  {
    key: 'forward-hold-route', source: 'Next-01',
    quote: 'The forward hold wasn’t far. Three turns past the galley she hadn’t used and the bunk she refused to sleep in properly.',
    category: 'adjacency', subject: 'route to Hold One ("the forward hold")',
    interpretation: 'The big hold — registry Hold One, still called "the forward hold" from an earlier deck plan — is reached from the bridge in a few turns past the galley. The turn count is a route claim the model should keep roughly honest once the hull massing settles.',
    evidence: 'explicit',
    claims: [{ kind: 'presence', target: 'holdOne' }],
  },
  {
  ...
  // ======== crew quarters ========
  {
    ...
    interpretation: '"Crew quarters" names the lower-deck residential cabin area as a whole (author ruling): the cabins only became crew quarters after there was a crew — B.O.B.’s designation arrived with Iri and Nova. Nova sleeps in an ordinary numbered cabin there; she left it 23 minutes before being found in pocket three.',
    evidence: 'explicit',
...
export const SEED_SCENES = [
  ...
  },
  {
    ...
    note: 'Presence-06: Iri on the bench getting stitched, Quenby working at the table, Nova leaning at the counter.',
    mannequins: [
      { character: 'iri', pos: [-0.95, 0, 12.1], rotY: Math.PI, pose: 'sit', supportHeight: 0.42, props: null },
      { character: 'quenby', pos: [-0.2, 0, 10.95], rotY: Math.PI * 0.75, pose: 'lean', props: null },
      { character: 'nova', pos: [-1.6, 0, 9.6], rotY: -Math.PI / 2, pose: 'lean', props: null },
    ],
    paths: [],
  },
  {
    name: 'The galley, measuring three',
    note: 'Presence-06, the author’s reading staged: a six-crew room crowded by two people’s habits meeting a third body. Quenby leaning at the counter run, Iri crouched at the cooling unit, Nova just inside the hatch with nowhere that is hers yet.',
    mannequins: [
      { character: 'quenby', pos: [-1.7, 0, 9.9], rotY: -Math.PI / 2, pose: 'lean', props: null },
      { character: 'iri', pos: [0.0, 0, 11.95], rotY: Math.PI * 0.35, pose: 'crouch', props: null },
      { character: 'nova', pos: [0.3, 0, 8.45], rotY: Math.PI, pose: 'stand', props: null },
    ],
    paths: [],
  },
];
