# WILD HUNTRESS — DESIGN DECISIONS LOG

This file records Wild Huntress design decisions established during the Between Ports visual-development process.
...
The Wild Huntress can be run by one person.

Implications for design:
- Critical flight/navigation/ship-control functions must be available to a solo operator.
- Additional bridge stations may improve workload, redundancy, specialization, or crew coordination, but must not be required for ordinary operation.
- The ship may still be designed to house and support a larger small crew.

---

## Residential / cabin area

### LOCKED — Quenby and Iri adjacency

Quenby's cabin and Iri's cabin share a wall.

Do not lose this adjacency when resolving the cabin-row layout or the existing Iri-quarters route conflict.

### LOCKED — Quenby's cabin is larger

Quenby's cabin should be one of the larger cabins in the residential area.
...
The galley is imagined with:
- a bench; and
- additional stools, chairs, or similar compact seating.
...
- a kettle;
- multiple cabinets/storage compartments;
- a sink;
- a food heating / hydration / synthesis appliance of some kind;
- some form of cold storage and/or hot holding.

Exact technology and arrangement remain OPEN and should be reconciled with existing prose evidence before being canonized.
...
### ROOMWRIGHT IMPLEMENTATION NOTE — The procedure bed (v25)

The author wanted a standalone, official medical procedure bed — and the realism question answered itself the way everything on this ship answers itself: **it converts.** A permanently standing walk-around bed is hospital floor-plan logic; a six-crew commercial hull weeks from help carries the *capability* instead, the way submarines and Antarctic stations do.

So the lower wall bed is now canonically a **certified medical procedure bed on recessed deployment rails**. Stowed flush against the wall, it is "the cot" — every frozen line stays true. Unlatch it and it slides out on twin deck tracks, casters lock, and the medic gets both sides of the patient; the articulated umbilical boom (power, med-gas, telemetry) follows it out from above the bed head and folds flat when it stows. The chair unhooks to its wall hanger; the upper fold-down cot latches flat. It is probably the single most over-spec fitting aboard — from whichever refit era certified her for crewed long-haul work — which is exactly why a century of owners who sold everything else never sold this.
...
### ROOMWRIGHT IMPLEMENTATION NOTE — The R. Vale crate chapter: the ship was already right (v31)

Fifth field test, and the best possible result: **the chapter agreed with geometry the model placed sessions before the chapter existed.** R. Vale's crate has stood in Pocket Three since the archaeology pass; the chapter opens the room, and everything holds — starboard position, the two-turn route from the bridge (validated at 6.2 m), the recessed-grip hatch that sticks from the wrong angle, the amber occupancy indicator, the seam light, the too-shallow/too-deep room where two people make a crowd. Even the pocket-crowd habit test's premise plays out on the page: Nova on the floor with the crate, Quenby making the room crowded by entering. And the acoustics hold the scene's secret: at drift-night, the Quenby–Nova conversation stays inside the pocket — which "What do I say if Iri asks?" quietly requires.

What the chapter added (v31):
- **The graphite pause-marks** are a real object on the widened spine wall by the support rail — Quenby's old lean spot, two turns from anywhere. Short strokes, not counts; one smeared soft, the last left clean; invisible head-on. The same notation R. Vale practiced on paper — except these are hers, and a pause does not belong to whoever learned how to draw it.
- **The crate carries its full inventory** in its note now: the stills, the margin-noted working references, the Helio's Belt strips and laminates, the throat-pickup case, the pause pages ("opens faster if I stop meaning it"), the false-scar overlay and pigmented skin patch, the R. Vale allocation tag — and the captain's storage ruling, honored by the model the way B.O.B. honors it: **location only, no item expansion** is the crate's registry entry, while the note (like the room itself) knows more than the manifest does.
...
### ROOMWRIGHT IMPLEMENTATION NOTE — Cabin One locked, the room that listens, and the plate-seven physics (v34)

**Cabin One is the second larger cabin (author lock)** — 2.4 × 2.0 m, Cabin Six's forward complement. It stayed unchosen through the two-crew years because it sits closest to the residential turn with the most traffic noise; in Volume 3 it becomes the mother-and-daughter cabin, where that noise turns into a feature. Pocket Two stays as it is (author confirmation): suspicious, unopened, loaded.

**Nova's listening array dresses the equipment crawl.** The crawl already held the cargo-panel desk and the folded insulation pad by name (mined earlier); the array chapter added its character: the receiver bank with the cloth-tied sticking knob, Iri's load-bearing invisibilities (cable weight, heat bleed, the non-broadcast filter, the bracket under Nova's elbow), the frequency slates (…DO NOT ANSWER), the patched lamp angled away from the doorway, the broken-crate side table. Field-tested: both figures place in the 1.8 × 1.45 room ("two steps in, one step sideways" — the model's own dimensions), and the room keeps its conversations, as a room that listens should. The suit-up beat grounds against the exterior kit locker, whose note has carried this chapter's exact tool list (tether clips, manual lock spanner, plate key, suit seals) since the fragments were first mined.

**Plate seven, assessed for realism (author question)** — verdict: fundamentally sound; three small canon choices make it airtight, recorded here:
1. **What the plates are:** manual reflective trim plates on the Huntress's hull — refit-era surfaces with mechanical angle locks (which is why the kit holds a *plate key* and *lock spanner*). On a ship that distrusts automation, manually latched trim is coherent engineering AND theme.
2. **Why they must be trimmed:** moorage albedo terms. A hull this size moored beside a light-farming station either shades or glares onto growing sections; stations that farm light impose reflectivity/trim requirements on moored ships. "Four degrees outside garden-side efficiency preference, one degree below hazard threshold" then reads exactly: preference = the crops' supplemental-light optimum; hazard = concentrated glare that burns leaves or blinds station optics. The correction windows pending station rotation are real geometry (glare and shadow move as the station turns).
3. **What the spike was:** a stellar proton event — the textbook EVA radiation hazard. Sudden onset, no local-instrument precursor (exactly matching "none of the external instruments registered variance before her call," which preserves Nova's warning as inexplicable), exposure through a suit, 36 hours of tissue stress, survivable. The plate is NOT the radiation source (reflected light isn't ionizing); the spike is environmental and coincidental in timing — the story's causal sin stays what the text says it is: taking the "acceptable" window instead of the optimal one.
Also sound as written: Iri anchoring (tether-tending is real two-person EVA procedure), "cleared for exterior work," the task being small and legitimately Quenby's ("you left it in the queue").

**Lore consolidated:** the Nine is **Iri-Nine** — and the "hastily carved nine" lives off-ship, in the station garden he started ("he wanted a place without orders"), consistent with the author's earlier confirmation. The garden is STATION-side; it is not the ship's domestic-stores/future-garden room — recorded here so the two are never conflated (see: the two-crates lesson). The garden station now carries real claims for any future exterior pass: moorage tether, garden-facing hull orientation, plate registry with plate seven tether-reachable from the personnel airlock.

33/33 tests green; v33→v34 migration verified; maps re-exported.
